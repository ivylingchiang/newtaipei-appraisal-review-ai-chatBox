...
def adjust(item, base_rank, comp_rank):
    ...
    即查表 matrix[比準地][比較標的]。**表5 區域因素與表4 個別因素同一方向**，共用本函式。
    方向以金山官方已填範本（查估書表範本 表4）校準：
    比準地 18m(稍優) ／ 比較標的 6m(稍劣) → 範本填 +5.00%，五筆非零細項皆同向。

    表6 不在本函式適用範圍：它調整的對象是宗地而非比較標的，角色互換後符號敘述亦相反
    （宗地條件較優 → 正，見 datasets/common/formulas.json 之 price_chain_table6），
    本專案未實作表6。
