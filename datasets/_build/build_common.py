...
FORMULAS = {
    ...
    "matrix": {
        ...
        "sign_convention": "比準地優於對造（比較標的或宗地，即對造級距數字較大）→ 修正率／修正百分比／差異率為正",
        "applies_to": "表5（區域因素）、表4 與表6（個別因素）同一方向，由 engine/grading.py 之 adjust() 共用實作；表6 之對造為宗地，查表方式不變（惟官方範本示範值與此相反，見 price_chain_table6.open_issue）。",
        "history": "曾一度將表5 改為相反方向（比準地 − 比較標的），理由為「同列中文等級"
                   "與數字正負須一致」；該判斷經承辦單位澄清為誤判，已回復單一方向。",
        "calibration": {
    ...
    "price_chain_table6": {
        ...
        "sign_convention": "比準地優於宗地（宗地級距數字較大）→ 差異率為正，與表5／表4 同一方向：差異率 = (宗地級距 − 比準地級距) × 級距差",
        "rule": "承辦單位認定之通則：比準地優於對造（比較標的或宗地）時，修正率／修正百分比／差異率一律為正，三張表不分方向。",
        "open_issue": {
            "summary": "官方 Excel 表6 範本的示範資料與上開通則相反，尚待承辦單位確認。",
            "source": "doc/table/*.xlsx 工作表「102表6宗地市價估計表格式」之官方示範資料"
                      "（表4、表5-1 之官方版面為空白範本，無示範值可反算）",
            "observation": "該示範資料中，宗地條件優於比準地者填正值、劣於比準地者填負值，"
                           "9 個細項 × 5 筆宗地全部一致，與通則相反。",
            "status": "本專案未實作表6（缺表7 宗地個別因素清冊），故不影響現有產出；"
                      "實作表6 前應先向承辦單位確認採何者。",
            "rows": [
                "8寬度：比準地 33M；宗地 15M(較窄→劣) 填 -0.01、57M 填 +0.01、60M 填 +0.02",
                "7面積：比準地 952㎡；宗地 530㎡ 填 -0.01、330㎡ 填 -0.02、4456㎡ 填 +0.04",
                "10形狀：比準地 方形；宗地 不規則形 填 -0.02",
                "11臨街情形：比準地 單面臨街；宗地 非臨街地 填 -0.02",
                "12地勢：比準地 平坦；宗地 低漥 填 -0.03、高亢 填 -0.05",
                "13道路種類：比準地 農路；宗地 無 填 -0.01",
                "20嫌惡設施：比準地 焚化場 2500M；宗地 3000M(較遠→優) 填 +0.02",
                "22使用分區：比準地 農牧用地；宗地 林業用地 填 -0.03",
            ],
        },
    },
    "rounding": {
